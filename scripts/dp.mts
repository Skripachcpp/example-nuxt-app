...
export function treeParse(tree: TreeParse[], level?: number, parentPre?: string, treeStr?: string[]): string[] {
 if (!level) level = 0
 ...
 tree.forEach(function (child, index) {
  ...
  treeStr?.push(''.concat(setPre(level, hasNext, parentPre)).concat(child.color
...
function findFiles(dirPath: string, ignorePaths?: string[], _arrayOfFiles?: string[]): string[] {
...
async function readComponents(pathFull: string): Promise<CmpByPath> {
 ...
 for (const pathFull of files) {
  ...
  readTasks.push(readFileAsync(pathFull).then((a) => {
...
const importsMatched = content.match(new RegExp(rxImports, 'g'))
  ...
  for (const importMatched of importsMatched) {
   ...
   if (importPathArray?.[0] == '~') {
    ...
    if (importPathArray?.[1] != 'components' && importPathArray?.[1] != 'shared') continue
   ...
   if (imp.pathFull && fs.existsSync(imp.pathFull)) {
    readTasks.push(readFileAsync(imp.pathFull).then((a) => imp.textOfImportedFile = a.toString()))
...
function initChildren(cmpElement: CmpElement, level = 0, parentsAll: CmpElement[] = []) {
  ...
  if (parentsAll.includes(cmpElement)) {
   ...
   errorAccumulator.add([
    ...
    color0(prnts.map((parent) => pathToRelativePath(parent.pathFull)).join(' => ')),
  ...
  for (const imp of (cmpElement.imports || [])) {
   ...
   if (impCmp == null) impCmp = componentsCmpByPath[importPath + '/index.tsx']
 ...
 function initParents(cmpElement: CmpElement, level = 0, parentsAll: CmpElement[] = []) {
  parentsAll = [...parentsAll]
  if (parentsAll.includes(cmpElement)) {
   const color0 = chalk.hex('#CD5C5C')
   const color1 = chalk.hex('#FF2400')
 ...
 function initReverseTree(cmpElement: CmpElement, level = 0, parentsAll: CmpElement[] = []) {
  parentsAll = [...parentsAll]
  if (parentsAll.includes(cmpElement)) {
   const color0 = chalk.hex('#CD5C5C')
   const color1 = chalk.hex('#FF2400')
...
function pathToComponentName(path: string) {
 ...
 function mapTsxElement2ConsoleTree(tsxElement: CmpElement, level = 0): ConsoleTree[] {
  ...
  for (const chd of (tsxElement.children || [])) {
   ...
   const element: ConsoleTree = { name, children: mapTsxElement2ConsoleTree(chd, level + 1) }
   consoleTreeElements.push(element)

   // подсвечиваем root элементы
   // там можно ссылаться на элементы в папке, но это редкость
   if (chd.pathFull.replace(__dirname + '/', '').replace('/index.tsx', '')?.split('/').length <= 2)
...
cmpUsed[children.pathFull] = true
 ...
 for (const pathFull in cmpNotUsed) {
  ...
  const pathRelativeArray = pathToRelativePath(pathFull).split(path.sep).filter((p) => p)
